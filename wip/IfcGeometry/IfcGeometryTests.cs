...
";
        ...
        // TEMP:
        public static async Task OutputParquet(IfcBinaryGeometry bg)
        {
            var dir = new DirectoryPath(Path.GetTempPath()).RelativeFolder("ifc-test");
            dir.CreateAndClearDirectory();

            {
                var dt = new DataTableBuilder("points");
                dt.AddColumn(bg.PointIds, nameof(bg.PointIds));
                dt.AddColumn(bg.PointXs, nameof(bg.PointXs));
                dt.AddColumn(bg.PointYs, nameof(bg.PointYs));
                dt.AddColumn(bg.PointZs, nameof(bg.PointZs));
                await dt.WriteParquetAsync(dir);
            }

            {
                var dt = new DataTableBuilder("faces");
                dt.AddColumn(bg.FaceIds, nameof(bg.FaceIds));
                dt.AddColumn(bg.FaceLoopOffsets, nameof(bg.FaceLoopOffsets));
                await dt.WriteParquetAsync(dir);
            }

            {
                var dt = new DataTableBuilder("faceLoops");
                dt.AddColumn(bg.FaceLoops, nameof(bg.FaceLoops));
                await dt.WriteParquetAsync(dir);
            }

            {
                var dt = new DataTableBuilder("loops");
                dt.AddColumn(bg.FaceIds, nameof(bg.LoopIds));
                dt.AddColumn(bg.FaceLoopOffsets, nameof(bg.LoopPointOffset));
                await dt.WriteParquetAsync(dir);
            }

            {
                var dt = new DataTableBuilder("loopPoints");
                dt.AddColumn(bg.FaceIds, nameof(bg.LoopPoints));
                await dt.WriteParquetAsync(dir);
            }

            var totalSize = dir.GetDirectorySizeInBytes();
            Console.WriteLine($"Total parquet output size = {totalSize:N0}");
        }
        ...
        {
            var filePath = dir.RelativeFile(table.Name + ".parquet");
            var stream = filePath.OpenWrite();
            await WriteParquetAsync(table, stream, level, method);
            stream.Flush();
            stream.Close();
            await stream.DisposeAsync();

            var temp = await ReadParquetAsync(filePath);
            Debug.Assert(temp.Columns.Count == table.Columns.Count);
            for (var i = 0; i < temp.Columns.Count; i++)
            {
                var c1 = table.Columns[i];
                var c2 = temp.Columns[i];
                Debug.Assert(c1.Count == c2.Count);
                for (var j = 0; j < c1.Count; j++)
                {
                    var v1 = c1[j];
                    var v2 = c2[j];
