...
namespace Ara3D
{
    public static class G3SharpBridge
    {
        ...
        public static IReadOnlyList<Vector3d> NearestPoints(this TriangleMesh3D self, IReadOnlyList<Vector3d> points)
        {
            var tree = self.ToG3Sharp().AABBTree();
            return points.Select(tree.NearestPoint);
        }

        public static IReadOnlyList<TriangleMesh3D> ToAra3DMeshes(this List<DMesh3> meshes)
            => meshes.Select(ToAra3D);
       ...
       public static void WriteFile(this DMesh3 mesh, string filePath, WriteOptions opts)
        ...
        public static IReadOnlyList<Vector3> ToAra3D(this DVector<double> self)
        {
            return (self.Length / 3).Select(i => new Vector3((float)self[i * 3], (float)self[i * 3 + 1], (float)self[i * 3 + 2]));
        }

        public static Point3D ToAra3D(this Vector3d self)
        {
            return new Vector3((float)self.x, (float)self.y, (float)self.z);
        }

        public static Bounds3D ToAra3D(this AxisAlignedBox3d box)
        {
            return (box.Min.ToAra3D(), box.Max.ToAra3D());
        }

        public static TriangleMesh3D ToAra3D(this DMesh3 self)
        {
            var verts = self.Vertices().Select(ToAra3D).ToList();
            var indices = self.Triangles().Select(f => new Integer3(f.a, f.b, f.c)).ToList();
        ...
        public static Vector3d ToVector3D(this Point3D self)
            => new Vector3d(self.X, self.Y, self.Z);
        ...
        public static DMesh3 ToG3Sharp(this TriangleMesh3D self)
        {
            ...
            var indices = self.CornerIndices();
